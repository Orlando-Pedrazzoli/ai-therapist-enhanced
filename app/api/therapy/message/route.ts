...
import { SessionEncryption, DataEncryption, getAuditLog } from '@/lib/security/encryption';
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (crisisAnalysis.level === 'CRITICAL') {
      ...
      return NextResponse.json({
        message: 'Por sua segurança, detectamos sinais que requerem atenção profissional imediata. Por favor, entre em contato com um dos serviços de emergência: CVV (188), SAMU (192) ou procure o CAPS mais próximo. Sua vida tem valor e há pessoas prontas para ajudar.',
        crisis: true,
        crisisLevel: 'CRITICAL',
    ...
    if (crisisAnalysis.level === 'HIGH' || crisisAnalysis.level === 'MEDIUM') {
      systemPrompt = promptSystem.createCrisisPrompt(message, crisisAnalysis.level);
    } else {
...
`;
    ...
    if (aiCrisisCheck.level === 'CRITICAL' || aiCrisisCheck.level === 'HIGH') {
      ...
      const fallbackMessage = 'Percebo que você está passando por um momento difícil. É importante lembrar que você não está sozinho. Existem pessoas e recursos disponíveis para apoiá-lo. Como posso ajudar você a encontrar o suporte necessário neste momento?';
      
      // Store fallback message
      const encryptedAiMessage = sessionEncryption.encryptMessage(fallbackMessage);
      await prisma.message.create({
        data: {
          sessionId: therapySession.id,
          role: 'assistant',
    ...
    if (error instanceof Error && error.message.includes('SAFETY')) {
      return NextResponse.json({
        message: 'Desculpe, não posso responder a essa mensagem por razões de segurança. Se você está passando por dificuldades, considere buscar ajuda profissional.',
        suggestProfessionalHelp: true,
        error: 'safety_block',
      });
    }
